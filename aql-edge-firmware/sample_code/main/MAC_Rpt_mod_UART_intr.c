...
// original ESP32 code - modified UART handler...
// look for 
...
//////////////////////////////////////////
//
// added custom character detect
//
//////////////////////////////////////////

static int UART_ISR_ATTR uart_find_chars_from_last(uint8_t *buf, int length, uint8_t *pat_chr, uint8_t pat_num, uint8_t *pat_rslt)
{
	char i;
	int cnt = 0;
    int len = length;
...
//internal isr handler for default driver code.
static void UART_ISR_ATTR uart_rx_intr_handler_modofied(void *param)
{
    uart_obj_t *p_uart = (uart_obj_t *) param;
    uint8_t uart_num = p_uart->uart_num;
    int rx_fifo_len = 0;
...
// get UART buffer data using len...
                uart_hal_read_rxfifo(&(uart_context[uart_num].hal), p_uart->rx_data_buf, &rx_fifo_len);
                uint8_t pat_chr = 0;
                uint8_t pat_num = 0;
                int pat_idx = -1;
#ifdef NEW_CHAR_DET_CODE
				int pat_chars_found = 0;
#endif
				
// get pattern info from OS
                uart_hal_get_at_cmd_char(&(uart_context[uart_num].hal), &pat_chr, &pat_num);

#ifdef NEW_CHAR_DET_CODE
// get eol characters (0x0A, '>')
// find pattern
                uart_event.size = rx_fifo_len;
                pat_idx = uart_find_chars_from_last(p_uart->rx_data_buf, rx_fifo_len - 1, pat_chk, pat_num, pat_rslt);
#endif
...
#ifdef NEW_CHAR_DET_CODE
                else if (pat_chars_found) 
					{
                    uart_hal_clr_intsts_mask(&(uart_context[uart_num].hal), UART_INTR_CMD_CHAR_DET);
                    uart_event.type = UART_EVENT_MAX+1;
                    uart_event.size = rx_fifo_len;
					} 
#endif
				else 
